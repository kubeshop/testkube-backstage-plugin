import {
  TESTKUBE_LABELS,
  TESTKUBE_ENVIRONMENTS,
  TESTKUBE_ORGANIZATION,
...
export const getTestkubeOrganization = (entity: Entity): string => {
  ...
  if (!value) {
    throw new Error(
      "'testkube.io/organization' annotation is missing in the entity",
    );
  }
  return value;
};
...
export const getTestkubeEnvironments = (entity: Entity): string[] => {
  ...
  if (!value) {
    throw new Error(
      "'testkube.io/environments' annotation is missing in the entity",
    );
  }
  return value
    .split(',')
