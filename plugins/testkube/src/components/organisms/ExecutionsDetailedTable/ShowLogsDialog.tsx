...
export const ShowLogsDialog: React.FC<ShowLogsDialogProps> = ({
  ...
  const loadLog = async (stepRef: string) => {
    try {
      setLoadingLog(true);
      await sleep(1000);
      const log = await TestkubeAPI.getTestWorkflowExecutionLog(
        workflowName,
        executionId,
      );
      ...
      if (element.children) {
        const childrenRows = element.children.map(children => {
          return (
            ...
            >
              {checkStepStatus(
                children.name || children.category || 'Undefined',
                execution.result?.steps[element.ref || ''].status,
              )}
            </ListItem>
          );
        });
        return (
          <div>
            ...
            >
              {checkStepStatus(
                element.name || element.category || 'Undefined',
                execution.result?.steps[element.ref || ''].status,
              )}
            </ListItem>
            {childrenRows}
    ...
    return (
      ...
      >
        ...
        >
          {checkStepStatus(
            'Initializing',
            execution.result?.initialization.status,
          )}
        </ListItem>
        {rows}
      </List>
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const execution = await TestkubeAPI.getTestWorkflowExecutionById(
        workflowName,
        executionId,
      );
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <DialogActions>
          <Button color="primary" onClick={closeLogDialog}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
